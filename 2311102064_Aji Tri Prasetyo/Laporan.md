...
Pada tugas **Pertemuan 13** ini, praktikum berfokus pada penerapan **State Management Provider** dan **Local Notifications** pada framework Flutter. Aplikasi yang dibangun adalah sebuah aplikasi sederhana berupa _counter_ (penghitung) yang menampilkan nilai angka, di mana setiap kali angka tersebut bertambah, akan muncul notifikasi lokal pada perangkat.
...
3. **`MyHomePage`** (`StatelessWidget`):
   ...
   - Menampilkan teks identitas mahasiswa.
...
Berikut adalah seluruh _source code_ (kode sumber) dari aplikasi yang memuat logika State Management, inisialisasi Notifikasi Lokal, serta antarmuka (UI), yang diimplementasikan pada berkas `lib/main.dart`:
...
3. **Pengelolaan Konfigurasi Android Modern** dengan mengaktifkan fitur `coreLibraryDesugaringEnabled` pada Gradle dan menambahkan _permission_ Android modern (`POST_NOTIFICATIONS`) agar aplikasi berjalan optimal pada OS terbaru.
